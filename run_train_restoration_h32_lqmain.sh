...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_DIR}/output_JiT-image-to-image-lqmain}"
...
LOG_FILE="${LOG_DIR}/train_restoration_h32_lqmain_${RUN_ID}.log"
LATEST_LOG_LINK="${LOG_DIR}/train_restoration_h32_lqmain_latest.log"
...
echo "JiT image-to-image LQ-main restoration training starting..."
...
TRAIN_CMD=(
  "${JIT_PYTHON}" -u main_jit_restoration_lqmain.py
  --model "${MODEL_NAME}"
  --img_size "${IMG_SIZE}"
  --data_path "${DATA_PATH}"
...
if [[ "${NUM_GPUS}" != "1" ]]; then
  TRAIN_CMD=(
    ...
    main_jit_restoration_lqmain.py
    --model "${MODEL_NAME}"
    --img_size "${IMG_SIZE}"
    --data_path "${DATA_PATH}"
    --qwen_model_path "${QWEN_MODEL_PATH}"
...
if ! "${TRAIN_CMD[@]}" 2>&1 | tee -a "${LOG_FILE}"; then
  echo "LQ-main restoration training failed. Last log lines:"
  tail -n 30 "${LOG_FILE}"
  exit 1
fi

echo "LQ-main restoration training finished."
echo "Output dir: ${OUTPUT_DIR}"
