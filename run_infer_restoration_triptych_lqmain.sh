...
LOG_FILE="${LOG_DIR}/infer_restoration_triptych_lqmain_${RUN_ID}.log"
LATEST_LOG_LINK="${LOG_DIR}/infer_restoration_triptych_lqmain_latest.log"
...
echo "JiT image-to-image LQ-main inference starting..."
...
if ! "${JIT_PYTHON}" -u infer_jit_restoration_lqmain.py \
  ...
  echo "LQ-main restoration inference failed. Last log lines:"
  tail -n 30 "${LOG_FILE}"
  exit 1
fi
